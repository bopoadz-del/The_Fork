...
| U1 | document upload path (large files) | **FINDING (not fabrication)** | the 25MB DG2 baseline .xer 502s on `POST /v1/projects/{id}/documents` — the handler buffers the whole file into memory AND copies it to encrypt synchronously, exceeding the Render gateway timeout. Small/mid .xer upload fine (HTTP 201). Fix options: stream-to-disk + encrypt in a background task, or raise the gateway/worker timeout for uploads. | `projects.py:656-714` | a ≥25MB allowed file uploads without 502; histogram then runs on it |
