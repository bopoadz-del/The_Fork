...
| F4 | CLOSED | Historical `DEEPSEEK_*` values appeared in git history. The provider is removed from this repo (no env, no ladder, no restore). Treat any leftover key as revoked; do not add a new one. | Purged in-repo. |
