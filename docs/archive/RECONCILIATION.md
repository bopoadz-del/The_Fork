...
## Headline
The v2 corpus is **well-formed and DG2-scoped**, not the full Drive archive. Per-document chunking is healthy; the audit's "<2 chunks/file" alarm was a wrong-denominator artifact (6,178 *expected Drive files* vs 53 *actually-ingested docs*).

## Counts (canonical store = `chunks_v2`)
| project | docs | chunks |
|---|---|---|
| dg2_infra_pack_1 | 39 | 9,857 |
| curated_kb (GK) | 8 | 605 |
| ha_long_xanh_2 | 6 | 40 |
| **total** | **53** | **10,502** |
...
## Completeness (4a)
- **DG2 package (the pilot core): ingested** — 39 docs / 9,857 chunks, incl. the DD-2022-175 demolition contract, DD-2023-118 Vol 1-3, PMC/PSA contracts, drawings, programmes.
- **Coverage gap:** only 53 docs are in v2. The broader Drive archive (a prior `drive_archive` of ~2,997 docs / ~139,949 chunks per session memory) is **NOT** in this DB — there is no `drive_archive` table. Individual non-DG2 project shells (Ha Long Xanh, etc.) are near-empty (UI: "still being indexed"). **If the pilot requires the full multi-project Drive corpus, that ingestion has not run into v2.** PARKED — needs a scoped re-ingest decision (pilot may be DG2-only by design).
...
## Tier 2/3 (4f)
Not separately tracked in v2. Only tier-1 DG2 package + GK notes present. Tier 2/3 Drive folders: NOT ingested into v2 (see Coverage gap). PARKED.
