...
- Env: `CEREBRUM_DOMAIN_KITS=construction` (T1), `MASTER_CORPUS_SOURCE_PROJECT_ID=dg2_infra_pack_1`, `AGENT_TIMING_LOG=1`.
...
| T4 | **Corpus coverage** | Only 53 docs in v2 (DG2 pkg + GK + a little Ha Long Xanh). Full Drive archive NOT in v2 (no `drive_archive` table). Non-DG2 project shells near-empty. | Decide pilot scope (DG2-only?) or re-ingest tier-2/3. |
...
| T5 | **Retrieval precision on v2** | Live-path recall eval: all 15 Qs **HIT (retrieved=5)** — recall is fine. But the **top chunk for every DG2 question is one FIDIC GK note (`980e19f6`)** — GK notes crowd out project-specific clauses → LLM says "could not find [DG2-specific]". **VALIDATED the auditor's warning:** enabling cfg7 knobs (MARGIN=0.15/BOOST=0.1/CAP=2) makes `retrieve_with_filter` return **EMPTY on v2** — the old-index tuning does NOT transfer. **Decision: keep knobs OFF on prod** (retrieval works, GK-dominated) — do NOT ship cfg7 (breaks v2). | **Re-sweep knobs on v2** (grid + eval referee, per T5); GK demotion must be re-derived, not copied. |
...
| T6b | **Attachment reasoning** | Files DO upload+index (uploaded RFPs present in dg2). But `[attached: X]` marker isn't parsed server-side and RFPs under-extracted (2 chunks) → "work on the attached file" weak. | Wire `[attached:X]`→fetch that doc/route to document_engine; fix RFP extraction. |

## Open human decisions (for Chadi)
1. **SENTRY_DSN** (T2) — provide it to restore observability.
2. **Pilot corpus scope** (T4) — DG2-package-only, or ingest the full Drive archive into v2?
